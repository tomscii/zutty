...
:   -rv          Reverse video
:   -shell       Shell program to run (default: bash)
...
Boolean options (=-altScroll=, =-autoCopy=, =-glinfo=, =-help=, =-rv=,
=-quiet=, =-verbose=) do not expect an argument; the presence of these
options amounts to a setting of "true". Other options expect exactly
one argument, with the exception of =-e=, which must be the last
option, to be followed by the command line to run.
...
selection capability (primary and clipboard), and expect to be able to
paste into other programs that source their selection data from the
clipboard.
...
resembles the behaviour of =xterm=, and follows these steps:
...
consulted; if that is not set, the built-in default of =bash= is
