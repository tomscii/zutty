...
If enabled, mouse scroll up and down events on the alternate screen
buffer are converted to synthetic up- and down-arrow keypresses. This
might prove to be a convenient way of moving up/down one at a time
in programs where one usually navigates with the keyboard arrows.

Another, similar effect is that the keys normally used to scroll the
terminal page history (Shift+PageUp, Shift+PageDown) will generate the
same synthetic up- and down-arrow keypresses as above, only in
multiples, to scroll half the amount of scrollable lines. This might
be convenient in the same situations, but for larger jumps.
...
=-altScroll= for enabling synthetic up- and down-arrow keypress
events).
